...
public class LoginTest extends BaseTest {
    ...
    void testLoginSuccess() {
        loginPage.navigate();
        loginPage.login("validUser@example.com", "ValidPassword123!");
        // After successful login, user should be redirected (no error alert shown)
        WebElement success = wait.until(ExpectedConditions.visibilityOfElementLocated(loginPage.getSuccessLocator()));
        assertTrue(success.isDisplayed(), "Login should be successful with valid credentials");
    }
    ...
    void testLoginEmptyCredentials() {
        loginPage.navigate();
        loginPage.login("", "");
        WebElement error = wait.until(ExpectedConditions.visibilityOfElementLocated(loginPage.getValidationErrorLocator()));
        assertTrue(error.isDisplayed(), "Validation error should be displayed for empty credentials");
    }

    @ParameterizedTest(name = "CSV Inline: {0} / {1} => {2}")
    @Order(4)
    @CsvSource({
            "validUser@example.com, ValidPassword123!, success",
    ...
    void testLoginCsvInline(String username, String password, String expected) {
        ...
        if (expected.equals("success")) {
            assertTrue(result.isDisplayed(), "Login should succeed for valid credentials");
        } else {
            assertTrue(result.getText().toLowerCase().contains("invalid")
                    || result.isDisplayed(), "Error message should be displayed for invalid credentials");
        }
    }
